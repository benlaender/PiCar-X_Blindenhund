...
def key_listener():
    ...
    while running:
        ...
        if key == 'p':
            if paused.is_set():
                paused.clear()
                # Add logic to resume car movement if necessary
                print("Script resumed by user input.")
            else:
                paused.set()
                px.stop()  # Physically stop the car when pausing
                print("Script paused by user input. Press 'p' again to resume.")
        elif key == 'q':
            running = False
            paused.set()  # Ensure the main loop exits if it's waiting on paused
            px.stop()
